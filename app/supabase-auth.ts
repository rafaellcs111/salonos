...
export async function upsertSupabaseUser(email: string, password: string, displayName: string) {
  ...
  if (result.error) {
    throw new Error(`Não foi possível salvar a senha do usuário: ${result.error.message}`);
  }
}
...
export async function deleteSupabaseUser(email: string) {
  ...
  for (let page = 1; page <= 10; page += 1) {
    ...
    if (existing) {
      ...
      if (result.error) throw new Error(`Não foi possível remover a credencial: ${result.error.message}`);
      return;
    }
