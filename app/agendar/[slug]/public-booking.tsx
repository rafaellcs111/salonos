...
export default function PublicBooking({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    fetch(`/api/storefront/${encodeURIComponent(slug)}`)
      ...
      .catch(() => setError("Esta empresa nÃ£o estÃ¡ disponÃ­vel para agendamentos."));
  }, [slug]);
  ...
  const slots = useMemo(() => {
    ...
    for (let minute = start; minute + duration <= end; minute += duration) {
      ...
      const now = new Date();
      const alreadyPassed = date === localDate(now) && minute <= now.getHours() * 60 + now.getMinutes();
      if (!booked.includes(slot) && !overlapsBreak && !overlapsAppointment && !alreadyPassed) result.push(slot);
    }
    return result;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!store || !service || !barber || !date || !time) {
      setError("Escolha serviÃ§o, profissional, data e horÃ¡rio.");
      return;
    }
    ...
    setError(data?.error || "NÃ£o foi possÃ­vel confirmar o agendamento.");
  }

  if (error && !store) return <main className="public-booking unavailable"><div><span className="chosen-mark">B</span><h1>Agenda indisponÃ­vel</h1><p>{error}</p></div></main>;
  ...
  if (success) {
    const message = `OlÃ¡! Acabei de agendar ${service} com ${barber} para ${formatDate(date)}, Ã s ${time}. Meu nome Ã© o informado no agendamento.`;
    return <main className={`public-booking theme-${store.tenant.theme}`}><section className="public-success"><span className="success-icon">âœ“</span><span className="section-kicker">AGENDAMENTO CONFIRMADO</span><h1>Nos vemos em breve!</h1><p>{service} com {barber}, dia {formatDate(date)} Ã s {time}.</p><div><strong>{store.tenant.name}</strong><small>{store.tenant.city} Â· {store.tenant.phone}</small></div><a className="whatsapp-button full" href={whatsappUrl(store.tenant.phone, message)} target="_blank" rel="noreferrer"><MessageCircle aria-hidden="true" /> Confirmar pelo WhatsApp</a><button className="outline-button full" onClick={() => window.location.reload()}>Fazer outro agendamento</button></section></main>;
  }

  return <main className={`public-booking theme-${store.tenant.theme}`}><header><div className="public-brand">{store.tenant.logoUrl ? <img className="tenant-public-logo" src={store.tenant.logoUrl} alt={`Logo ${store.tenant.name}`} /> : <span className="chosen-mark">{store.tenant.name[0]}</span>}<span><strong>{store.tenant.name}</strong><small>{store.tenant.businessType === "salon" ? "SalÃ£o" : "Barbearia"} Â· {store.tenant.city}</small></span></div><span>Agendamento online por SalonOS</span></header>
    <form className="public-booking-card" onSubmit={submit}>
      <div className="public-heading"><span className="section-kicker">AGENDE SEU HORÃRIO</span><h1>Escolha seu atendimento</h1><p>ConfirmaÃ§Ã£o imediata, sem pagamento online.</p></div>
      <fieldset><legend>1. ServiÃ§o</legend><div className="public-options">{store.services.map((item) => <button type="button" key={item.name} className={service === item.name ? "selected" : ""} onClick={() => setService(item.name)}><span><strong>{item.name}</strong><small>{item.duration} min</small></span><b>R$ {item.price}</b></button>)}</div></fieldset>
      <fieldset><legend>2. Profissional</legend><div className="public-options compact">{eligibleBarbers.map((item) => <button type="button" key={item.name} className={barber === item.name ? "selected" : ""} onClick={() => setBarber(item.name)}><span className="avatar">{item.name[0]}</span><strong>{item.name}</strong></button>)}</div>{!eligibleBarbers.length && <p className="empty-message">Nenhum profissional atende este serviÃ§o no momento.</p>}</fieldset>
      <fieldset><legend>3. Data e horÃ¡rio</legend><div className="public-dates">{dates.map((item) => { const value = localDate(item); return <button type="button" className={date === value ? "selected" : ""} key={value} onClick={() => setDate(value)}><small>{item.toLocaleDateString("pt-BR", { weekday: "short" })}</small><strong>{item.getDate()}</strong></button>; })}</div><div className="public-times">{slots.map((slot) => <button type="button" className={time === slot ? "selected" : ""} key={slot} onClick={() => setTime(slot)}>{slot}</button>)}</div></fieldset>
      ...
      {error && <p className="error">{error}</p>}<button className="gold-button full" disabled={sending || !time}>{sending ? "Confirmando..." : "Confirmar agendamento â†’"}</button>
    </form>
  </main>;
}
