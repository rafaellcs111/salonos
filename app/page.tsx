...
export default function Home() {
  ...
  async function saveConfig(next: BusinessConfig) {
    ...
    if (response.ok) {
      ...
      setSaveNotice("Não foi possível salvar");
    }
  }
  ...
  const visibleAdminNav = useMemo(() => navItems.filter((item) => {
    ...
    if (item === "Financeiro") return permissions.finance;
    if (item === "Estoque") return permissions.settings && tenantPlan !== "starter";
    if (item === "Equipe" || item === "Serviços") return permissions.settings;
    return true;
  ...
  return (
    <main className={`app-shell ${view === "admin" && tenantBusinessType === "salon" ? "manager-white" : ""}`}>
      <aside className="sidebar">
        <Logo />
        <div className="workspace"><span className="workspace-icon">{view === "master" ? "S" : tenantName[0]}</span><span><strong>{view === "master" ? "SalonOS Master" : tenantName}</strong><small>{view === "master" ? "Super administrador" : accountRole === "staff" ? "Acesso da equipe" : "Gestão da unidade"}</small></span><b>⌄</b></div>
...
function ClientsContent({ tenantId }: { tenantId: string }) {
  ...
  return <div className="clients-page">
    <div className="settings-intro clients-intro">
      ...
      <label className="client-search"><span>⌕</span><input aria-label="Buscar clientes" placeholder="Buscar por nome ou telefone" value={search} onChange={(event) => setSearch(event.target.value)} /></label>
    </div>
...
function AgendaContent({ tenantId, config }: { tenantId: string; config: BusinessConfig }) {
  ...
  return <div className="agenda-page">
    <div className="agenda-toolbar">
      ...
      <div className="agenda-controls"><button className="outline-button" onClick={() => moveWeek(-1)}>←</button><button className="outline-button" onClick={() => setWeekStart(startOfWeek(new Date()))}>Hoje</button><button className="outline-button" onClick={() => moveWeek(1)}>→</button><button className="gold-button compact" onClick={() => setCreating(true)}>+ Novo horário</button></div>
    </div>
...
function DashboardContent({ mini = false, config = defaultConfig, tenantId = "chosen" }: { mini?: boolean; config?: BusinessConfig; tenantId?: string }) {
  ...
  return (
    <div className={`dashboard ${mini ? "mini" : ""}`}>
      ...
      />}
      {!mini && <section className="panel quick-actions"><h2>Ações rápidas</h2><div><button>+ Agendamento</button><button>♙ Novo cliente</button><button>▦ Bloquear horário</button><button>↗ Registrar entrada</button></div></section>}
    </div>
  );
}
...
function MasterContent({ section, onNavigate }: { section: string; onNavigate: (section: string) => void }) {
  ...
  if (section === "Planos") {
    const plans = [
      { id: "starter", name: "Starter", description: "O essencial para organizar um negócio pequeno", count: planCounts.starter || 0, features: ["Agenda e link de agendamento", "Cadastro básico de clientes", "Serviços e horários", "Até 3 profissionais"] },
      { id: "pro", name: "Pro", description: "Gestão completa para negócios em crescimento", count: planCounts.pro || 0, features: ["Tudo do Starter", "Controle de estoque e produtos", "Financeiro e relatórios", "Equipe, permissões e até 10 profissionais"] },
